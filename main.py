...
# ==========================================
    ...
    if df.empty: 
        st.info("اس تاریخ اور فلٹر کے مطابق کوئی ریکارڈ نہیں ملا۔")
    else:
...
# ==========================================
    ...
    s_list = [f"{s[0]} بنت {s[1]}" for s in c.execute("SELECT name, father_name FROM students").fetchall()]
    if not s_list: 
...
# ==========================================
    ...
    with st.form("leave"):
        ...
        if st.form_submit_button("ارسال کریں"):
            ...
            conn.commit()
            st.success("درخواست مہتممہ کو بھیج دی گئی ہے!")
    ...
    if not reqs: 
        st.info("اس وقت چھٹی کی کوئی نئی درخواست نہیں ہے۔")
    ...
    if not df_lv.empty: 
        st.markdown(generate_html_print(df_lv, "اساتذہ کی رخصت کا ریکارڈ"), unsafe_allow_html=True)
