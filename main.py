...
# ==========================================
# ماڈیول 1: ڈیش بورڈ (صرف ایڈمن کے لیے - لائیو ڈیٹا کے ساتھ)
# ==========================================
if m == "📊 ڈیش بورڈ (Overview)":
    st.header("📊 جامعہ کا تفصیلی ڈیش بورڈ")
    today_str = str(date.today())
    
    # ڈیٹا بیس سے لائیو اعداد و شمار نکالنا
    total_students = c.execute("SELECT COUNT(*) FROM students").fetchone()[0]
    total_teachers = c.execute("SELECT COUNT(*) FROM teachers WHERE name != 'admin'").fetchone()[0]
    teachers_present = c.execute("SELECT COUNT(DISTINCT t_name) FROM t_attendance WHERE manual_date=?", (today_str,)).fetchone()[0]
    pending_leaves = c.execute("SELECT COUNT(*) FROM leave_requests WHERE status='پینڈنگ'").fetchone()[0]
    
    col1, col2, col3, col4 = st.columns(4)
    with col1: st.markdown(f"<div class='dashboard-card'><div class='card-title'>کل طلباء</div><div class='card-value'>{total_students}</div><small>جامعہ میں زیر تعلیم</small></div>", unsafe_allow_html=True)
    with col2: st.markdown(f"<div class='dashboard-card'><div class='card-title'>اساتذہ (آج حاضر)</div><div class='card-value'>{teachers_present} / {total_teachers}</div><small>آج کی حاضری</small></div>", unsafe_allow_html=True)
    with col3: st.markdown(f"<div class='dashboard-card'><div class='card-title'>رخصت کی درخواستیں</div><div class='card-value'>{pending_leaves}</div><small>زیرِ التوا (Pending)</small></div>", unsafe_allow_html=True)
    with col4: st.markdown(f"<div class='dashboard-card'><div class='card-title'>فیس سسٹم</div><div class='card-value'>جَلد</div><small>نیا فیچر آنے والا ہے</small></div>", unsafe_allow_html=True)

    st.markdown("---")
    col_left, col_right = st.columns([2, 1])
    with col_left:
        st.subheader("⚡ فوری ایکشنز")
        c1, c2 = st.columns(2)
        if c1.button("📝 مہتمم پینل (رخصت دیکھیں)"): st.session_state.quick_nav = "🏛️ مہتمم پینل (رخصت)"; st.rerun()
        if c2.button("🎓 نیا طالب علم رجسٹر کریں"): st.session_state.quick_nav = "⚙️ انتظامی کنٹرول"; st.rerun()
        
    with col_right:
        st.subheader("📢 نوٹس بورڈ")
        st.markdown("""<div class='notice-board'><b>🔔 اعلانات:</b><br>امتحانات کا ریکارڈ بروقت اپڈیٹ کریں اور حاضری یقینی بنائیں۔<br><b>📅 آج کی تاریخ:</b> """ + today_str + """</div>""", unsafe_allow_html=True)
