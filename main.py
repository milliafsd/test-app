...
def init_db():
    # پہلے پرانے ٹیبلز میں ضروری نئے کالم شامل کریں
    ...
    # ---------- ڈیفالٹ ڈیٹا ----------
    default_levels = [
        ("متوسطہ سال اول", 1), ("متوسطہ سال دوم", 2), ("متوسطہ سال سوم", 3),
        ("اولیٰ", 4), ("درجہ ثانیہ", 5), ("درجہ ثالثہ", 6),
        ("درجہ رابعہ", 7), ("درجہ خامسہ", 8), ("درجہ سادسہ", 9),
        ("موقوف علیہ", 10), ("دورہ حدیث", 11)
    ]
    ...
    # ---------- مائیگریشن: پرانے ریکارڈز کو نئے سٹرکچر سے جوڑیں ----------
        ...
        with st.container():
            ...
            if st.button("داخل ہوں"):
                ...
                if res:
                    ...
                    if u == "admin": st.session_state.user_type = "admin"
                    elif role == "aasri": st.session_state.user_type = "aasri"
                    else: st.session_state.user_type = "teacher"
...
if st.session_state.user_type == "admin":
    menu = ["📊 ایڈمن ڈیش بورڈ", "📊 یومیہ تعلیمی رپورٹ", "🎓 امتحانی نظام", "📜 ماہانہ رزلٹ کارڈ",
            ...
            "📋 عملہ نگرانی و شکایات", "📢 نوٹیفیکیشنز", "📈 تجزیہ و رپورٹس", "🏆 ماہانہ بہترین طلباء", "⚙️ بیک اپ & سیٹنگز"]
...
if selected == "📊 ایڈمن ڈیش بورڈ" and st.session_state.user_type == "admin":
    ...
    if dept_filter in ["تمام", "حفظ"]:
        ...
        try:
            ...
            if not hifz_df.empty:
                if sel_teacher != "تمام": hifz_df = hifz_df[hifz_df['استاد'] == sel_teacher]
    ...
    if dept_filter in ["تمام", "قاعدہ"]:
        ...
        try:
            ...
            if not qaida_df.empty:
                if sel_teacher != "تمام": qaida_df = qaida_df[qaida_df['استاد'] == sel_teacher]
    ...
    if dept_filter in ["تمام", "درسِ نظامی", "عصری تعلیم"]:
        ...
        """
        params = [d1, d2]
        if sel_teacher != "تمام": query += " AND g.t_name = ?"; params.append(sel_teacher)
        if dept_filter != "تمام": query += " AND g.dept = ?"; params.append(dept_filter)
        try:
            gen_df = pd.read_sql_query(query, conn, params=params)
            conn.close()
            if not gen_df.empty: combined_df = pd.concat([combined_df, gen_df], ignore_index=True)
        except: pass
...
# ==================== 9. درسِ نظامی اور ماسٹر ٹائم ٹیبل (نئے سیکشنز) ====================
    ...
    with tab3:
        ...
        with st.form("new_session"):
            s_name = st.text_input("سیشن کا نام")
            ...
            if st.form_submit_button("نیا سیشن بنائیں"):
                if s_name:
                    conn = get_db_connection()
                    c = conn.cursor()
                    if is_active: c.execute("UPDATE academic_sessions SET is_active=0")
    ...
    col1, col2 = st.columns(2)
    with col1: filter_day = st.selectbox("دن فلٹر کریں", ["تمام"] + days)
    with col2:
    ...
    """
    params = [session_id]
    if filter_day != "تمام": query += " AND mt.day = ?"; params.append(filter_day)
    if filter_level != "تمام": query += " AND dl.id = ?"; params.append(level_dict[filter_level])
    ...
    with st.expander("➕ نیا پیریڈ شامل کریں"):
        with st.form("add_master_period"):
            day = st.selectbox("دن", days)
            period_no = st.number_input("پیریڈ نمبر", 1, 10, 1)
...
# ==================== 10. استاد ڈیش بورڈ (ماسٹر ٹائم ٹیبل پر مبنی) ====================
...
# ==================== 12. مشترکہ سیکشنز (حاضری، رخصت، پاسورڈ) ====================
