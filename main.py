...
if st.session_state.user_type == "admin":
    menu = ["📊 یومیہ تعلیمی رپورٹ", "🖨️ ٹریکنگ و پرنٹ رپورٹ", "🎓 امتحانات و نتائج", "📜 ماہانہ رزلٹ کارڈ", "🕒 استاذہ کا ریکارڈ", "🏛️ مہتمم پینل (رخصت)", "⚙️ انتظامی کنٹرول"]
else:
...
if m == "⚙️ انتظامی کنٹرول":
    ...
    with tab1:
        st.subheader("نئ استانی شامل کریں")
        with st.form("add_teacher"):
            ...
            if st.form_submit_button("محفوظ کریں"):
                ...
                conn.commit(); st.success( استانی رجسٹر ہو گئی !")
                st.rerun()
    ...
    with tab2:
        st.subheader("نئ طالبہ شامل کریں")
        with st.form("add_student"):
            s_name = st.text_inpu.("نام طالبہ")
            f_name = st.text_input("دختر")
            ...
            if st.form_submit_button("محفوظ کریں") and t_assign:
                ...
                conn.commit(); st.success("طالبہ رجسٹر ہو گئی!")
                st.rerun()
...
# ==========================================
    ...
    with st.sidebar:
        ...
        sel_t = st.selectbox("استاذہ منتخب کریں", t_list)
    ...
    if sel_t !="تمام استاذہ": query += " AND t_name = ?"; params.append(sel_t)
...
# ==========================================
    ...
    st.header("🖨️ انفرادی طالبہ ٹریکنگ اور پرنٹ")
    ...
    if not s_list: st.warning("کوئی طالب علم موجود نہیں۔")
        ...
        sel_s = st.selectbox("طالبہ منتخب کریں", s_list)
        sn, fn = sel_s.split(" دختر ")
...
# ==========================================
    ...
    if st.session_state.user_type == "teacher":
        ...
        if my_students:
            sel_s = st.selectbox("طالبہ ", my_students)
        ...
        else: st.warning("کوئی طالبہ نہیں۔")
    else:
        ...
        with tab2:
            df_exams = pd.read_sql_query("SELECT id as 'ID', s_name as 'نام', f_name as 'دختر', para_no as 'پارہ', total as 'نمبر', grade as 'گریڈ', end_date as 'تاریخ' FROM exams WHERE status='مکمل'", conn)
...
# ==========================================
    ...
    sel_t = st.selectbox("استاذہ", t_list)
...
# ==========================================
    ...
    df_att = pd.read_sql_query("SELECT id as 'ID', t_name as 'استانی', manual_date as 'تاریخ', manual_time as 'وقتِ آمد' FROM t_attendance ORDER BY manual_date DESC", conn)
